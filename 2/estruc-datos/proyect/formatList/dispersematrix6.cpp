...
void DisperseMatrix :: printMatrix2(){
    int i, j;
    for(i=0;i<disperse.size();i++){
        cout << "[" <<"Row: "<< i ;
        for(j=0;j<disperse[i].size();j++){
            cout << "(" << disperse[i][j].first << ", " << disperse[i][j].second << "),";
        }
        cout << "]" << endl;
    }
    cout << endl;
...
//     int i, j, h,count_j, count_h, indiceJ;
//     int filas, columnas;
//     bool flag_i ,flag_j, flag_h, flag;

//     filas = max(maxFila, m.maxFila) , columnas = max(maxCol , m.maxCol);
//     filas1 = 0, suma = 0, multiplicacion = 0, aux_i = 1, cantidadValores = 0, flag_i = true; 
//     ans.maxFila = maxFila, ans.maxCol = m.maxCol;
...
//     for(i = 0; i < filas && flag_i; i++){
//         deque <pair<int,int>> auxVec;
//         columnas1 = 0 ,filas2 = 0, cantCerosFilas = 0, flag_j = true, indiceJ = 0;
//         count_j = 0;

//         if(disperse[i].empty() || i >= maxFila ){
//             flag_j = false;
//         }
//         for(j = 0; j < filas && flag_j; j++){
//             flag = true;           
//             columnas2 = 0, numEliminaciones = 0;
//             // Obtener valor1 
//             if(disperse[i][count_j].second == columnas1)
//                 valor1 = disperse[i][count_j].first;
//             else
//                 valor1 = 0;                
//             count_h = 0; 
//             for(h = 0; h < columnas; h++){

//                 if(m.disperse[j].empty() || j >= m.maxFila){
//                     flag = false;
...
//                 }
//                 else{
//                     if( m.disperse[j][count_h].second == columnas2 )
//                         valor2 = m.disperse[j][count_h].first;
//                     else
//                         valor2 = 0; 
//                 }               
//                 if(i != aux_i){
...
//                     pair<int,int> auxPair;
//                     auxPair.first = multiplicacion;
//                     auxPair.second = columnas2;
//                     auxVec.push_back(auxPair);
//                 }
//                 else{
//                     multiplicacion = valor1* valor2;
//                     suma = multiplicacion + auxVec[h - numEliminaciones].first;
//                     auxVec[h - numEliminaciones].first = suma;
//                     if(j == (filas -1)){
//                         if(auxVec[h - numEliminaciones].first == 0){
//                             auxVec.erase(auxVec.begin() + h - numEliminaciones);
//                             numEliminaciones++;
//                         }
//                     }
...
//                 // cout << valor1 << " " << valor2 << " i " << i << " j " << j << " h " << h << endl;
//                 // cout << valor1 << " " << valor2 << "suma: " << suma << ", "
//                 //         << "multiplicacion: " << multiplicacion << ", "
//                 //         << "count_h: " << count_h << ", "
//                 //         << "numEliminaciones: " << numEliminaciones << endl;
//                 // cout<<" vec : " ;
//                 // for(int l = 0.; l < auxVec.size(); l++){
//                 //     cout << "( " << auxVec[l].first << "," << auxVec[l].second <<" )";
//                 // }
...
DisperseMatrix DisperseMatrix::getTranspose() {
    ...
    ans.disperse.resize(ans.maxFila);

    for (int i = 0; i < disperse.size(); ++i) {
        for (int j = 0; j < disperse[i].size(); ++j) {
            int targetRow = disperse[i][j].second;

            if (targetRow < ans.disperse.size()) {
                ans.disperse[targetRow].push_back(make_pair(disperse[i][j].first, i));
            } else {
                while (targetRow >= ans.disperse.size()) {

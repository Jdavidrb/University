...
int DisperseMatrix::get(int i, int j) {
    ...
    for (size_t h = 0; h < disperse[i].size() && !flag; ++h) {
...
list<pair<int, int>> DisperseMatrix:: getDisperseRowList(int j) {
    ...
    for(i=0;i<disperse[j].size();i++){
        ans.push_back(make_pair(disperse[j][i].first, disperse[j][i].second));
    }
...
list<pair<int, int>> DisperseMatrix::getDisperseColList(int j) {
    ...
    for(i=0;i<disperse.size();i++){
        for(h=0;h<disperse[i].size();h++){
            if(disperse[i][h].first == j){
                ans.push_back(make_pair(disperse[i][h].first, i));
            }
        }
    }
...
list<pair<int, int>> DisperseMatrix::getRowList(int j) {
    ...
    if(j < maxFila){
        for(i=0;i<maxCol;i++){
            ...
            ans.push_back(make_pair(valor, i));
        }
    }
...
/* void DisperseMatrix::assign(int i, int j, int valor){ //REVISAR
    int h;
    if(i < maxFila && j < maxCol){
        ...
        if(flag && valor != 0){
            ...
            disperse[i].push_back(make_pair(valor, j));
        }
    }
} */
